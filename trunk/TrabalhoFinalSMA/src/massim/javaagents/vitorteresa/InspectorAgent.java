...
public class InspectorAgent extends Agent {
	...
	private void handleMessages() {
		...
		// LinkedList<LogicBelief> beliefs = getAllBeliefs("lastAction");
		//
		// if(!beliefs.isEmpty()){
		// LogicBelief lastAction = beliefs.getFirst();
		// if(lastAction.getParameters().firstElement().equals("inspect")){
		//
		// }
		//
		// }
	...
	private void handlePercepts() {
		...
		for (Percept p : percepts) {
			if (p.getName().equals("lastAction")) {
				if (p.getParameters().getFirst().equals("inspect")) {
					inspected = true;
				}
			}
		}
		...
		for (Percept p : percepts) {
			...
			if (p.getName().equals("step")) {
				...
				if (inspected) {
					addBelief(Mars2011Util.perceptToBelief(p));
				}
	...
	private Action planInspect() {
		...
		for (LogicBelief v : visible) {
			...
			if (vertices.contains(pos) == false)
				continue;
			adjacentNum++;
			enemies.add(new LogicBelief("enemy", pos));
